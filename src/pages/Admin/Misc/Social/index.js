...
const Social = (props) => {
    ...
    const handleSelectItem = (element) => {
        setName(element.social_content)
    }

    useEffect(() => {
        if(data) {
            data.user_social.map((element, index) => {
                                ...
                                id && id.map((element,i) => {
                                    return(
                                        <div key={i} onClick={() => handleCurrentId(i)}>
                                            {data && data.user_social.map((value, index) => {
                                                if(element === value.id){
                                                    return(
                                                        <div key={index} className={clsx(classes.items, currentId === i && classes.selectItem)} onClick={() => handleSelectItem(value)}>
                                                            <div>{value.social_content}</div>
                                                            <div onClick={() => handleDeleteItem("Social", value.id)}>X</div>
                                                        </div>
                                                    )
                                                }
                                            })}
