...
const Transaction = (props) => {
    ...
    const handleSelectItem = (element) => {
        setName(element.trans_type)
    }

    useEffect(() => {
        if(data) {
            data.trans_type.map((element, index) => {
                                ...
                                id && id.map((element,i) => {
                                    return(
                                        <div key={i} onClick={() => handleCurrentId(i)}>
                                            {data && data.trans_type.map((value, index) => {
                                                if(element === value.id){
                                                    return(
                                                        <div key={index} className={clsx(classes.items, currentId === i && classes.selectItem)} onClick={() => handleSelectItem(value)}>
                                                            <div>{value.trans_type}</div>
                                                            <div onClick={() => handleDeleteItem("TransactionType", value.id)}>X</div>
                                                        </div>
                                                    )
                                                }
                                            })}
